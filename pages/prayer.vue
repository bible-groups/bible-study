<template>
  <div class="max-h-screen overflow-y-auto">
    <div class="container mx-auto px-4 py-8">
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center justify-between mb-2">
                  ...
                  <div class="flex items-center space-x-2 ml-4">
                    ...
                    <span class="text-sm text-gray-400">{{ prayer.author }}</span>
                  </div>
                </div>
      ...
      <!-- 페이지네이션 -->
      <div v-if="allFilteredPrayers.length > 0 && totalPages > 1" class="flex justify-center mt-8">
      ...
      <div class="bg-white rounded-lg shadow-xl w-[100vh] max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div class="p-6 border-t border-gray-200">
          <div class="flex justify-center space-x-4">
            <button @click="updatePrayer" :disabled="updating" class="px-6 py-2 bg-indigo-500 text-white rounded-lg hover:bg-indigo-400 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center space-x-2 cursor-pointer">
            ...
            <button @click="deletePrayer" :disabled="deleting" class="px-6 py-2 bg-red-500 text-white rounded-lg hover:bg-red-400 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center space-x-2 cursor-pointer">
...
const addNewPrayer = async () => {
  try {
    ...
    const prayerCollection = collection($db, 'prayer')
    const authorName = user.value?.email || ''
    const docRef = await addDoc(prayerCollection, { title: newPrayer.value.title, body: newPrayer.value.body, author: authorName, createdAt: serverTimestamp() })
    const local = { id: docRef.id, title: newPrayer.value.title, body: newPrayer.value.body, author: authorName, createdAt: new Date().toLocaleDateString('ko-KR', { year:'numeric', month:'long', day:'numeric' }) }
