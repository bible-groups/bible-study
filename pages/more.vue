<template>
  <div class="max-h-screen overflow-y-auto">
    <div class="container mx-auto px-4 py-8">
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center justify-between mb-2">
                  ...
                  <div class="flex items-center space-x-2 ml-4">
                    ...
                    <span class="text-sm text-gray-400">{{ memo.author }}</span>
                  </div>
                </div>
      ...
      <!-- 페이지네이션 -->
      <div v-if="allFilteredMemos.length > 0 && totalPages > 1" class="flex justify-center mt-8">
        <nav class="flex items-center space-x-1">
          ...
          <button v-if="totalPages > 6 && currentPage < totalPages - 2" @click="goToPage(totalPages)" :class="['w-8 h-8 flex items-center justify-center rounded-md text-sm font-medium transition-colors', currentPage === totalPages ? 'bg-indigo-100 text-indigo-700 font-semibold' : 'text-indigo-600 hover:bg-indigo-50 hover:text-indigo-700']">{{ totalPages }}</button>
          <button :disabled="currentPage >= totalPages" @click="goToPage(currentPage + 1)" :class="['w-8 h-8 flex items-center justify-center rounded-md text-sm font-medium transition-colors', currentPage >= totalPages ? 'text-gray-400 cursor-not-allowed' : 'text-indigo-600 hover:bg-indigo-50 hover:text-indigo-700']"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg></button>
        </nav>
      </div>

      <!-- 데이터가 없을 때 -->
      <div v-if="!loading && allFilteredMemos.length === 0" class="text-center py-12">
      ...
      <div class="bg-white rounded-lg shadow-xl w-[100vh] max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div class="p-6 border-t border-gray-200">
          <div class="flex justify-center space-x-4">
            <button @click="updateMemo" :disabled="updating" class="px-6 py-2 bg-indigo-500 text-white rounded-lg hover:bg-indigo-400 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center space-x-2 cursor-pointer"><svg v-if="updating" class="w-4 h-4 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path></svg><svg v-else class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg><span>{{ updating ? '수정 중...' : '수정' }}</span></button>
            <button @click="deleteMemo" :disabled="deleting" class="px-6 py-2 bg-red-500 text-white rounded-lg hover:bg-red-400 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center space-x-2 cursor-pointer"><svg v-if="deleting" class="w-4 h-4 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path></svg><svg v-else class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg><span>{{ deleting ? '삭제 중...' : '삭제' }}</span></button>
            <button @click="closeModal" class="px-6 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-400 transition-colors flex items-center space-x-2 cursor-pointer"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg><span>창닫기</span></button>
          </div>
        </div>
      </div>
    </div>
...
const addNewMemo = async () => {
  try { adding.value=true; const { $db } = useNuxtApp(); const { collection, addDoc, serverTimestamp } = await import('firebase/firestore'); const memosCollection = collection($db,'more'); const authorName = user.value?.email || ''; const docRef = await addDoc(memosCollection,{ title:newMemo.value.title, body:newMemo.value.body, author:authorName, createdAt: serverTimestamp() }); const local={ id:docRef.id, title:newMemo.value.title, body:newMemo.value.body, author:authorName, createdAt:new Date().toLocaleDateString('ko-KR',{year:'numeric',month:'long',day:'numeric'}) }; memos.value.unshift(local); newMemo.value={ title:'', body:'' }; viewMode.value='list'; alert('메모가 성공적으로 추가되었습니다.') } catch(e){ console.error('메모 추가 실패:', e); alert('메모 추가 중 오류가 발생했습니다.') } finally { adding.value=false } }
