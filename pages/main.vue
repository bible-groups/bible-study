          ...
          <div class="bg-white rounded-lg border border-gray-200 overflow-hidden mb-6">
            ...
            <div v-if="loading" class="text-center py-12">
              <div class="flex flex-col items-center justify-center">
                ...
                <p class="text-gray-500">묵상 목록을 불러오는 중...</p>
              </div>
            </div>
          ...
          <div class="bg-white rounded-lg border border-gray-200 overflow-hidden mb-6">
            ...
            <div v-if="blogLoading" class="text-center py-12">
              <div class="flex flex-col items-center justify-center">
                ...
                <p class="text-gray-500">블로그 목록을 불러오는 중...</p>
              </div>
            </div>
        ...
        <div class="main-right-search-area mt-15">
          <div class="bg-white rounded-lg">
            ...
            <div class="px-6 py-4">
              ...
              <!-- 검색 옵션 라디오 버튼들 -->
              <div class="hidden xl:flex gap-4 mt-12 justify-center ">
         ...
         <div class="p-6 border-t border-gray-200">
           ...
           <div class="flex justify-center space-x-4 mt-10">
             ...
             >
               ...
               <span>{{ blogUpdating ? '수정 중...' : '수정' }}</span>
             </button>
             ...
             >
               ...
               <span>{{ blogDeleting ? '삭제 중...' : '삭제' }}</span>
             </button>
...
const loadMeditationsFromFirebase = async () => {
  try {
    ...
    const q = query(meditationsCollection) // orderBy 제거하여 인덱스 문제 해결
...
const filteredMeditations = computed(() => {
  ...
  if (isBibleSearchActive.value && selectedBook.value) {
    ...
    result = result.filter(meditation => {
      ...
      // 성경장이 선택되었을 경우 추가 검증
      if (chapterNumber) {
        // 제목에서 ':' 기준으로 앞부분을 추출하여 장 번호 확인
        const colonIndex = title.indexOf(':')
        if (colonIndex !== -1) {
          const beforeColon = title.substring(0, colonIndex)
          // 성경책명 + 장번호 패턴 확인 (예: "창세기 1", "마태복음 5" 등)
          const pattern = `${bookName} ${chapterNumber}`
          return beforeColon.includes(pattern)
        } else {
          // ':'이 없는 경우 성경책명 + 장번호 패턴이 제목에 포함되어 있는지 확인
          const pattern = `${bookName} ${chapterNumber}`
  ...
  // 텍스트 검색 (성경 검색이 비활성화된 경우만)
    ...
    const searchTerm = meditationSearchTerm.value.toLowerCase()
    result = result.filter(meditation => 
      meditation.title.toLowerCase().includes(searchTerm) ||
      meditation.body.toLowerCase().includes(searchTerm)
    )
  }
